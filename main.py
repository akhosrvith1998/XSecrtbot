...
def update_user_started(user_id):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
async def handle_inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_record or not user_record[4]:  # started field
        deep_link = f"https://t.me/{BOT_USERNAME}?start=deep_link"
        results = [
            InlineQueryResultArticle(
                id="start_bot",
                title="لطفا قبل از شروع روی این پیام کلیک کن🤌🏼",
                input_message_content=InputTextMessageContent(
                    "برای استفاده از ربات، ابتدا ربات را استارت کنید.\n"
                    "روی دکمه زیر کلیک کنید و سپس گزینه Start را بزنید."
                ),
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("شروع ربات", url=deep_link)]
                ]),
                description="استارت ربات برای استفاده"
            )
        ]
    ...
    try:
        ...
        if member.status not in ["member", "administrator", "creator"]:
            ...
            results = [
                InlineQueryResultArticle(
                    id="join_sponsor",
                    title="لطفا در کانال اسپانسر عضو شوید",
                    input_message_content=InputTextMessageContent(
                        "برای استفاده از ربات باید در کانال اسپانسر عضو باشید."
                    ),
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("عضویت در کانال", url=f"https://t.me/{SPONSOR_CHANNEL}")]
                    ]),
    ...
    if len(parts) > 1:
        ...
        whisper_id = f"whisper_{recipient_id or recipient_username}_{timestamp}"
        results.append(InlineQueryResultArticle(
            id=whisper_id,
    ...
    if data.startswith("view_"):
        ...
        if query.from_user.id == whisper[2]:  # recipient_id
            ...
            if whisper[5] > 0:  # view_count
                ...
                new_text += "هنوز ندیده 😐\n"
                new_text += f"تعداد فضول ها: {whisper[7]} نفر"
            
            keyboard = []
            # فقط اگر حذف نشده باشد دکمه‌ها را نمایش بده
            if not whisper[8]:  # deleted
                keyboard.append([InlineKeyboardButton("پاسخ 💭", callback_data=f"reply_{whisper_id}")])
                # فقط فرستنده می‌تواند حذف کند
                if query.from_user.id == whisper[1]:  # sender_id
                    keyboard.append([InlineKeyboardButton("حذف 🤌🏼", callback_data=f"delete_{whisper_id}")])
